library(ggplot2)
#setwd("Desktop/BMI")
mat <- read.table("Documents/Parent of Origin/Paper/maternal_LDL_850134.ped")
pat <- read.table("Documents/Parent of Origin/Paper/paternal_LDL_850134.ped")
fam <- read.table("Documents/Parent of Origin/Paper/maternal_LDL_850134.fam")

mat <- read.table("Documents/Parent of Origin/Paper/maternalLDL_10662534.ped")
pat <- read.table("Documents/Parent of Origin/Paper/paternalLDL_10662534.ped")
fam <- read.table("Documents/Parent of Origin/Paper/maternalLDL_10662534.fam")

phen <- read.table("Documents/Parent of Origin/needtoorganize/Re__LDL_residuals/LDLcombSNP.ph-cvt", header = T)
...
ggplot(a2, aes(gtype, residuals, fill= gtype)) +
  ...
  scale_fill_brewer(name = "Genotype",
                    breaks = c("G(m)G(p)",  "G(m)T(p)", "T(m)G(p)", "T(m)T(p)"),
                    labels = c(expression(G[m]~G[p]),
                               expression(G[m]~A[p]),
                               expression(A[m]~G[p]),
                               expression(G[m]~G[p])),
                    palette = "PRGn")+
...
ggplot(a2, aes(mat, residuals, fill= mat)) +
  ...
  scale_fill_manual(name = "Maternal Allele",
                    breaks = c("G",  "T"),
                    labels = c(expression(G[m]),
                               expression(T[m])),
                    values = c("#de2d26", "#fc9272"))+
  labs(y = "LDL residuals\n", x = "Maternal Allele", title = "LDL by Parent of Origin Alleles\n") 
...
ggplot(a2, aes(pat, residuals, fill= pat)) +
  ...
  scale_fill_manual(name = "Paternal Allele",
                    breaks = c("G",  "T"),
                    labels = c(expression(G[p]),
                               expression(T[p])),
                    values = c("#3182bd", "#9ecae1"))+
  labs(y = "LDL residuals\n", x = "Paternal Allele", title = "LDL by Parent of Origin Alleles\n") 
